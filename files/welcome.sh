...
function retropie_welcome() {
    ...
    if [[ -f "/opt/vc/bin/vcgencmd" ]]; then
        if gpuTempC=$(/opt/vc/bin/vcgencmd measure_temp); then
            ...
            gpuTempC=$(/usr/bin/nvidia-smi -q -d temperature | grep 'GPU Current Temp' | awk -F " " '{print $5}')
        fi
    fi
    local df_out=()
    local line
    ...
    for i in "${!logo[@]}"; do
        ...
        case "$i" in
            ...
            9)
                ...
                ;;
            10)
                out+="${fgwht}credit to The RetroPie Project, https://retropie.org.uk"
                ;;
        esac
        out+="\n"
    done
    echo -e "\n$out\n${fggrn} Temperature Nvidia GPU $(/usr/bin/nvidia-smi -q -d temperature | grep 'GPU Current Temp' | awk -F " " '{print $5}')"
}
retropie_welcome
